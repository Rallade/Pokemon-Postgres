            ...
            try {
                if (target_data.id == datum.pokemon_id) {
                    switch (datum.stat_id) {
                        case '1':
                            target_data.HP = HP_base_to_stat(parseInt(datum.base_stat), target_data.level)
                            break
                        case '2':
                            target_data.Attack = base_to_stat(parseInt(datum.base_stat), target_data.level)
                            break
                        case '3':
                            target_data.Defense = base_to_stat(parseInt(datum.base_stat), target_data.level)
                            break
                        case '4':
                            target_data.SpAttack = base_to_stat(parseInt(datum.base_stat), target_data.level)
                            break
                        case '5':
                            target_data.SpDefense = base_to_stat(parseInt(datum.base_stat), target_data.level)
                            break
                        case '6':
                            target_data.Speed = base_to_stat(parseInt(datum.base_stat), target_data.level)
                            break
                    }
                }
            }
